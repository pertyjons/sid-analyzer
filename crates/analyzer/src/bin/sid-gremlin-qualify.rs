...
const DRIVER: &str = "Ben_Daglish/Gremlin";

#[derive(Parser)]
#[command(about = "Qualify Gremlin native extraction across a SID corpus")]
struct Cli {
    #[arg(long)]
    corpus: PathBuf,
...
fn identified_paths(cli: &Cli) -> Result<Vec<PathBuf>, AppError> {
    let db = PlayerDb::embedded();
    ...
    let rows: Vec<Result<Option<PathBuf>, AppError>> = paths
        ...
        .map(|path| {
            ...
            Ok((db.identify(&bytes) == Some(DRIVER)).then(|| path.clone()))
        })
        .collect();
...
fn summarize(results: &[QualificationResult]) -> QualificationSummary {
...
fn qualify(cli: &Cli) -> Result<QualificationReport, AppError> {
    ...
    let rows: Vec<Result<QualificationResult, AppError>> = identified
        .par_iter()
        .map(|path| qualify_one(&cli.corpus, path, cli.frames, timeout))
        .collect();
...
fn run() -> Result<(), AppError> {
    ...
    eprintln!(
        "Gremlin qualification: {}/{} accepted, {} structured in {:.2}s",
...
fn main() {
    if let Err(error) = run() {
        eprintln!("sid-gremlin-qualify: {error}");
        std::process::exit(1);
    }
}
